#!/usr/bin/env python3
"""
Instagram Reel Video Downloader and Content Extractor
This script downloads a video from an Instagram reel URL using yt-dlp
...
class InstagramContentExtractor:
    """Class to extract text content from Instagram posts."""
    
    def __init__(self, url):
        """Initialize with the Instagram URL."""
        self.url = url
    ...
    def extract_post_text(self):
        """Extract the text content from the Instagram post."""
        try:
            print(f"Fetching post content from: {self.url}")
            
            # First try using yt-dlp as it handles Instagram authentication better
            ...
            # If yt-dlp didn't work, try direct scraping (less reliable for Instagram)
            ...
            # Try to find text in any div with specific class patterns common in Instagram
            if not post_text:
                for div in soup.find_all(['div', 'span'], {'class': re.compile(r'(caption|text|content)')}):
            ...
            if post_text:
                ...
                print("Could not extract post text content through direct scraping.")
                return None
...
class InstagramVideoDownloader:
    """Class to download videos from Instagram using yt-dlp."""
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Download a video and extract text from an Instagram reel.')
    parser.add_argument('--url', type=str, default="https://www.instagram.com/reel/DJrd0sdJvq_/?igsh=MnB0ZG5kM2UzM2o2",
                        help='URL of the Instagram reel')
    ...
    content_extractor = InstagramContentExtractor(args.url)
    ...
    downloader = InstagramVideoDownloader(args.url)
